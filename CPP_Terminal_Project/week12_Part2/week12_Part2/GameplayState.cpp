...
bool GameplayState::Load() {
    ...
    m_pLevel = new Level();
	return m_pLevel->Load(m_LevelNames.at(m_currenLevel), m_player.GetXPositionPointer(), m_player.GetYPositionPointer());
}
...
bool GameplayState::Update(bool processInput) {
    ...
    if (m_beatLevel) {
        ...
        ++m_skipFrameCount;
        if (m_skipFrameCount > kFrameToSkip) {

            m_beatLevel = false;
            m_skipFrameCount = 0;

            ++m_currenLevel;
            if (m_currenLevel == m_LevelNames.size()) {
...
void GameplayState::DrawHUD(const HANDLE& console) {
    ...
    // Right Border
    CONSOLE_SCREEN_BUFFER_INFO cabi;
    GetConsoleScreenBufferInfo(console, &cabi);

    COORD pos;
    pos.X = m_pLevel->GetWidth() - 1;
    pos.Y = cabi.dwCursorPosition.Y;
